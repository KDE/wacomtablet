...
QString DBusTabletService::getDeviceName(const QString &tabletId, const QString& device) const
{
    ...
    if (type == NULL) {
        qCritical() << QString::fromLatin1("Unsupported device type '%1'!").arg(device);
        return unknown;
    }
...
QString DBusTabletService::getInformation(const QString &tabletId,const QString& info) const
{
    ...
    if (devinfo == NULL) {
        qCritical() << QString::fromLatin1("Can not get unsupported tablet information '%1'!").arg(info);
        return unknown;
    }
...
QString DBusTabletService::getProperty(const QString &tabletId, const QString& deviceType, const QString& property) const
{
    ...
    if (type == NULL) {
        qCritical() << QString::fromLatin1("Can not get property '%1' from invalid device '%2'!").arg(property).arg(deviceType);
        return QString();
    }
    ...
    if (prop == NULL) {
        qCritical() << QString::fromLatin1("Can not get invalid property '%1' from device '%2'!").arg(property).arg(deviceType);
        return QString();
    }
...
void DBusTabletService::setProperty(const QString &tabletId, const QString& deviceType, const QString& property, const QString& value)
{
    ...
    if (type == NULL) {
        qCritical() << QString::fromLatin1("Can not set property '%1' on invalid device '%2' to '%3'!").arg(property).arg(deviceType).arg(value);
        return;
    }
    ...
    if (prop == NULL) {
        qCritical() << QString::fromLatin1("Can not set invalid property '%1' on device '%2' to '%3'!").arg(property).arg(deviceType).arg(value);
        return;
    }
