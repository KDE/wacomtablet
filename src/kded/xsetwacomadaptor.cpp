...
const QString XsetwacomAdaptor::getProperty(const Property& property) const
{
    ...
    if (xsetproperty == NULL) {
        qCritical() << QString::fromLatin1("Can not get unsupported property '%1' using xsetwacom!").arg(property.key());
        return QString();
    }
    ...
    qDebug() << QString::fromLatin1("Reading property '%1' from device '%2' -> '%3'.").arg(property.key()).arg(d->device).arg(xsetwacomValue);
...
bool XsetwacomAdaptor::setProperty(const Property& property, const QString& value)
{
    ...
    qDebug() << QString::fromLatin1("Setting property '%1' to '%2' on device '%3'.").arg(property.key()).arg(value).arg(d->device);

    const XsetwacomProperty* xsetproperty = XsetwacomProperty::map(property);

    if (xsetproperty == NULL) {
        qCritical() << QString::fromLatin1("Can not set unsupported property '%1' to '%2' on device '%3' using xsetwacom!").arg(property.key()).arg(value).arg(d->device);
        return false;
    }
...
const QString XsetwacomAdaptor::convertParameter(const XsetwacomProperty& param) const
{
    ...
    if (rx.indexIn(modifiedParam, 0) != -1) {
        ...
        //qDebug() << QString::fromLatin1("Mapping tablet button %1 to X11 buton %2.").arg(hwButtonNumber).arg(kernelButtonNumber);
