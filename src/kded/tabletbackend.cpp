...
const QString TabletBackend::getProperty(const DeviceType& type, const Property& property) const
{
    ...
    if (!d->deviceAdaptors.contains(type)) {
        qCritical() << QString::fromLatin1("Could not get property '%1' from unsupported device type '%2'!").arg(property.key()).arg(type.key());
        return QString();
    }
    ...
    foreach(const DeviceType& deviceType, DeviceType::list()) {
        if (d->tabletInformation.hasDevice(deviceType)) {
            if (profile.hasDevice(deviceType)) {
                qDebug() << QString::fromLatin1("Setting profile '%1' on tablet '%2', device '%3'").arg(profile.getName()).arg(d->tabletInformation.get(TabletInfo::TabletName)).arg(deviceType.key());
                DeviceProfile deviceProfile = profile.getDevice(deviceType);
                setProfile(deviceType, deviceProfile);
            } else {
                qDebug() << QString::fromLatin1("Skipping '%1' settings as the current profile does not contain any settings for this device...").arg(deviceType.key());
            }
        } else {
            qDebug() << QString::fromLatin1("Skipping '%1' settings as the device does not support it...").arg(deviceType.key());
        }
    }
}
...
void TabletBackend::setProfile(const DeviceType& deviceType, const DeviceProfile& profile)
{
    ...
    if (!d->deviceAdaptors.contains(deviceType)) {
        qCritical() << QString::fromLatin1("Could not set profile on unsupported device type '%1'!").arg(deviceType.key());
        return;
    }
...
bool TabletBackend::setProperty(const DeviceType& type, const Property& property, const QString& value)
{
    ...
    if (!d->deviceAdaptors.contains(type)) {
        qCritical() << QString::fromLatin1("Could not set property '%1' to '%2' on unsupported device type '%3'!").arg(property.key()).arg(value).arg(type.key());
        return false;
    }
