...
xcb_input_get_device_property_reply_t* X11InputDevice::getPropertyData (const QString& property, X11InputDevice::Atom expectedType, int expectedFormat, long int nelements) const
{
    ...
    if (nelements < 1) {
        errWacom << QString::fromLatin1 ("Can not get XInput property '%1' as less than one element was requested!").arg(property);
        return NULL;
    }
...
bool X11InputDevice::setProperty(const QString& property, X11InputDevice::Atom expectedType, const QList< T >& values) const
{
    ...
    // check parameters
    if (!isOpen()) {
        errWacom << QString::fromLatin1 ("Can not set XInput property '%1' as no device was opened!").arg(property);
        return false;
    }
    ...
    if (!lookupProperty(property, propertyAtom)) {
        errWacom << QString::fromLatin1("Can not set unsupported XInput property '%1'!").arg(property);
        return false;
    }
